...
$(".cod").on('focusout',function(){
		...
		{		
			...
			}else
			{
				...
				//activar los input que correspondan
				$tr.children('td:eq(2)').children().children().prop('disabled',false);
				$tr.children('td:eq(3)').children().children().prop('disabled',false);
				$tr.children('td:eq(4)').children().children().prop('disabled',false);
				
				$tr.find("#codigo").prop('disabled',false);
				$tr.find("#precio_unitario").prop('disabled',false);
				$tr.find("#total_tbl").prop('disabled',false);
...
$(".cant").on('focus',function(){
	...
	if(total_stock<0)
	{
		$("#tbl_foot tr td:eq(0)").children().html('<span class="label label-important" style="font-size: 100%;"></span>');
	}else
	{
	$("#tbl_foot tr td:eq(0)").children().html('<span class="label label-info" style="font-size: 100%;"></span>');
	}
...
//TRABAJAR CON EL BTN PARA GUARDAR LOS DATOS
      ...
      function valores() {
            ...
            //	valor2.val(Dato2);
            valor3.val(Dato3);
			vd.find("#descripcion").prop('disabled',true).val("");			
			vd.find("#cantidad").prop('disabled',true).val("");
			vd.find("#precio_unitario").prop('disabled',true).val("");
			vd.find("#descuento").prop('disabled',true).val("");
			vd.find("#bodega").prop('disabled',true).val("");
			vd.find("#total_tbl").prop('disabled',true).val("");
      ...
      $(".btn-codigo").on('click', function() {
		...
		vd = $(this).parent().parent().parent().parent().parent().parent().parent();
            //valor=$(".cod_complete");
