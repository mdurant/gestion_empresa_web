...
public function clientes()
	...
	// Nuevo Mauricio > Area->Contabilidad->Asiento
public function documentos_contabilizar(){
...
public function obtieneasiento()
		{
		$Factura="";
		$query= "select * from easientos";
		$resul=mysql_query($query,conectar::con());
		while($row=mysql_fetch_assoc($resul))
		{
			$Factura=$row["voucher"];
...
{
	$salida=array();
	$query="SELECT CodeBar FROM cuentacontable";
	$res=mysql_query($query,conectar::con());
	while($row=mysql_fetch_assoc($res))
	{
		$salida[]=$row["Codigo"];
