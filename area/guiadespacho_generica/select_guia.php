...
public function trabajadores(){
	...
	$salida=array();
	$query="SELECT trabajador.id_trabajador, concat(nombres, ' ' ,apellidop, ' ', apellidom) as datos_trabajador FROM trabajador";
	$res=mysql_query($query,Conectar::con());
	while($row=mysql_fetch_assoc($res))
	{
		$salida[]=$row;
...
public function obtieneguia_generica()
	{
		$guia="";
...
public function eguiad($id)
{
		$salida=array();
		$query=<<<QUERY
		select * from eguiadespacho_general where IdEGuiaDespacho_General="$id"
QUERY;
		$res=mysql_query($query,Conectar::con());
		while($row=mysql_fetch_assoc($res))
		{
...
public function dguiad($id)
{
	$salida=array();
	$query=<<<QUERY
	select * from dguiadespacho_general where IdEGuiaDespacho_General="$id"
QUERY;
	$res=mysql_query($query,Conectar::con());
	while($row=mysql_fetch_assoc($res))
	{
