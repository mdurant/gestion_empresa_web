...
try
{
	...
	if($_GET["action"] == "list")
	{
		...
		$jtPageSize=$_GET["jtPageSize"];
            $seleccionar="(FechaTermino >= NOW()) and";
		$from="FROM
                ecotizacion
		...
		$where="WHERE $seleccionar $dat (customers.rut LIKE '%$product%' or empresa.RazonSocial LIKE '%$product%' or ecotizacion.Contador LIKE '%$product%')";
			...
			//comienaza la insercion
			 for($i=0;$i<sizeof($dat);$i++)
			 {
			 	...
			 	$iquery="INSERT INTO  dfactura (
                    ...
                    //PARA EL STOCK DE LOS PRODUCTOS
                    $produ=$dat[$i]["Codigo"];
                    $salida=array();
            		$queryST="SELECT
            				product.ProductName,
            				...
            				WHERE CodeBar= '$produ'";
            		$res=mysql_query($queryST,conectar::con());
            		while($row=mysql_fetch_assoc($res))
            		{
            			$salida[]=$row;
            		}
                    $stocks=$salida[0]["UnitsInStock"];
		          	$idep=$salida[0]["IDProduct"];
                    $stockNew=$stocks - $dat[$i]["Cantidad"];
                    $querysp="UPDATE product SET  UnitsInStock =  '$stockNew' WHERE  product.IDProduct ='$idep'";
	               	mysql_query($querysp,conectar::con());
                }
