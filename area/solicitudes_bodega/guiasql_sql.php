...
try
{
	...
	if($_GET["action"] == "list")
	{
        ...
        $dat="(fecha_sol BETWEEN '".$inicio."' AND '".$fin."') and";
        }
		...
		$from="FROM
		esolicitud
		INNER JOIN trabajador ON esolicitud.id_jefe = trabajador.id_trabajador
		AND esolicitud.id_operario = trabajador.id_trabajador
		INNER JOIN proyectos ON esolicitud.id_proyecto = proyectos.id_proyecto
		INNER JOIN dsolicitud ON esolicitud.id_esolicitud = dsolicitud.id_esolicitud
		AND dsolicitud.entregado_por = trabajador.id_trabajador
		INNER JOIN product ON dsolicitud.codigo_producto = product.CodeBar";
		$where="WHERE $dat (Traba LIKE '%$product%')  $radio";
...
QUERY;
	...
	dsolicitud.producto,
	dsolicitud.cantidad,
	dsolicitud.observacion,
	dsolicitud.entregado_por,
	product.ProductName,
	trabajador.id_trabajador,
	concat(trabajador.nombres, ' ' ,trabajador.apellidop) as Traba
	$from  $where
	ORDER BY $jtSorting
	$limit;		
