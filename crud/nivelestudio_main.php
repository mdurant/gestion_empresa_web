...
<h4>Maestro Nivel de Estudios</h4>
	...
	<tr>
	  <td width="60%">
		...
		<tr>
		  ...
		  <td><input type="text" id="nombreestudio" name="nombreestudio" style="width:100%" placeholder="" class="form-control input-sm ui-corner-all"></td>
		</tr>
	      </tbody>
	    </table>
	  </td>
...
<div id="jt_nivel" style="width: 100%;"></div>
...
<script type="text/javascript">
	...
	toolbar: {
	  items: [{
	    ...
	    click: function() {
	      ...
	      var $selectedRows = $('#jt_nivel').jtable('selectedRows');
	...
	actions: {
	  listAction: 'nivelestudio_sql.php?action=list',
	  createAction: 'nivelestudio_sql.php?action=create',
	  updateAction: 'nivelestudio_sql.php?action=update',
	  deleteAction: 'nivelestudio_sql.php?action=delete'
	},
	fields: {
	  IDNivel: {
	    key: true,
	...
	formCreated: function(event, data) {
      ...
      $('#jt_nivel').jtable('load');

      //buscar por clientes
      $('#btnBUSCAR').on('click', function(e) {
	e.preventDefault();
	$('#jt_nivel').jtable('load', {
	  nombreestudio: $('#nombreestudio').val(),
	...
	buttons: {
	  ...
	  "Eliminar": function() {
	    var $selectedRows = $('#jt_nivel').jtable('selectedRows');
	    $('#jt_nivel').jtable('deleteRows', $selectedRows);
	    $(this).dialog("close");
	  }
	}
      });
