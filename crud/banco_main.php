...
<head>
<meta charset="utf-8"/>
<title>Instituciones Bancarias / Financieras</title>
...
<h4>Instituciones Bancarias / Financieras</h4>
	...
	<tr>
	  <td width="60%">
		...
		<tr>
		  ...
		  <td><input type="text" id="nombrebanco" name="nombrebanco" style="width:100%" placeholder="" class="form-control input-sm ui-corner-all"></td>
		</tr>
	      </tbody>
	    </table>
	  </td>
...
<div id="jt_banco" style="width: 100%;"></div>
...
<script type="text/javascript">
	...
	toolbar: {
	  items: [{
	    ...
	    click: function() {
	      ...
	      var $selectedRows = $('#jt_banco').jtable('selectedRows');
	...
	actions: {
	  listAction: 'banco_sql.php?action=list',
	  createAction: 'banco_sql.php?action=create',
	  updateAction: 'banco_sql.php?action=update',
	  deleteAction: 'banco_sql.php?action=delete'
	},
	fields: {
	  id_banco: {
	    key: true,
	...
	buttons: {
	  ...
	  "Eliminar": function() {
	    var $selectedRows = $('#jt_banco').jtable('selectedRows');
	    $('#jt_banco').jtable('deleteRows', $selectedRows);
	    $(this).dialog("close");
	  }
	}
      });
