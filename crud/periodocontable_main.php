	...
	<tr>
	  <td width="60%">
		...
		<tr>
		  ...
		  <td><input type="text" id="periodocontable" name="periodocontable" style="width:100%" placeholder="" class="form-control input-sm ui-corner-all"></td>
		</tr>
	      </tbody>
	    </table>
	  </td>
...
<div id="jt_periodocontable" style="width: 100%;"></div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
      //Prepare jTable
      $('#jt_periodocontable').jtable({
	jqueryuiTheme: true,
	title: 'Listado',
	paging: true,
	pageSize: 10,
	sorting: true,
	defaultSorting: 'id_periodo ASC',
	selecting: true,
	//Enable selecting
	multiselect: true,
	...
	toolbar: {
	  items: [{
	    ...
	    click: function() {
	      ...
	      var $selectedRows = $('#jt_periodocontable').jtable('selectedRows');
	...
	actions: {
	  listAction: 'periodocontable_sql.php?action=list',
	  createAction: 'periodocontable_sql.php?action=create',
	  updateAction: 'periodocontable_sql.php?action=update',
	  deleteAction: 'periodocontable_sql.php?action=delete'
	},
	fields: {
	  id_periodo: {
	    key: true,
	  ...
	  id_autorizado: {
	    ...
	    edit: true,
	    list: true,
	    options: 'periodocontable_sql.php?action=autorizado'
	...
	formCreated: function(event, data) {
      ...
      $('#jt_periodocontable').jtable('load');

      //buscar por clientes
      $('#btnBUSCAR').on('click', function(e) {
	e.preventDefault();
	$('#jt_periodocontable').jtable('load', {
	  tipocontrato: $('#cuentascontables').val(),
	...
	buttons: {
	  ...
	  "Eliminar": function() {
	    var $selectedRows = $('#jt_periodocontable').jtable('selectedRows');
	    $('#jt_periodocontable').jtable('deleteRows', $selectedRows);
	    $(this).dialog("close");
	  }
	}
      });
